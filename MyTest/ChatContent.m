...
@property(retain,nonatomic)AVIMConversation *conver;
@end
...
- (void)viewDidLoad {
    ...
    //_conver=[[AVIMConversation alloc]init];
...
//            }else{
//                [self.client createConversationWithName:@"test" clientIds:@[@"james456"] callback:^(AVIMConversation *conversation, NSError *error) {
    AVIMMessage *message = [AVIMMessage messageWithContent:strtext];
                   [_conver sendMessage:message options:nil callback:^(BOOL succeeded, NSError *error) {
                  
                       if (succeeded) {                            NSLog(@"success send ");
                       }else{
